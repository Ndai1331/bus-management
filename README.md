...
Phase 2–4A currently includes server-side station scope, source-specific revenue validation, immutable issued/closed records, settlement/day-close advisory locking, closed-day adjustments with maker-checker approval, compliance expiry counts, server-side XLSX/PDF/HTML exports, and parking tariff/session lifecycle with idempotent parking receipts. Phase 4A hardening adds a station-safe composite receipt FK `(ParkingSessionId, StationId)`, legacy parking backfill, UTC/business-date validation (including rejection of future exits), explicit transactions for parking mutations and daily close, and tariff/charge snapshot fields in DTOs and outbox events. Export requires `HCS.BusManagement.Reports.Export`; the Blazor dashboard currently exposes revenue export links, while the other export types remain API routes. Migration `20260829133927_AddBusPhaseFourParking` is present; focused Bus Management tests are `27 passed` and the full solution is `383 passed`. Validation evidence and runtime limitations are recorded in [`docs/bus-management-v1.md`](./docs/bus-management-v1.md#migration-và-validation-evidence). PostgreSQL, Gateway/BFF, Keycloak and browser smoke tests remain runtime gates for applied migration/lock behavior, routing/session/permission claims, and end-to-end acceptance. The current branch is published at [Ndai1331/bus-management](https://github.com/Ndai1331/bus-management).
